var mysql = {
	listFriendId:function(userId) {
		var query = 'SELECT IF(u.id > r.user_one,r.user_one,r.user_two) AS friendId ' +
					'FROM yuki.user as u,yuki.relationship as r ' +
	...
	listFriendOnline: function(list) {
		var head= 'SELECT u.id, u.displayName, u.jlpt, u.level, u.picture, u.score,  ' +
					'(SELECT COUNT(*) FROM yuki.user WHERE yuki.user.score>=u.score) AS rank  ' +
					'FROM yuki.user as u , yuki.online as o  ' +
					'WHERE u.id = o.userId ' +
					'AND (';
		var tails = ') ' +
					'GROUP BY o.userId ';
	...
	// q.isTest as "question_isTest"
	// FROM yuki.report as r 
	// LEFT JOIN yuki.question as q
