...
module.exports = {
	...
	rankJoinGame: function(req, res){
		...
		let SetUpPlayer = function(){
	    	return new Promise(function(fullfill, reject){
	    		Game.create({status:false, user_one:id1, user_two:id2, type:1}).exec(function(err,game){
					if(err) return reject(err);
					gameId= game.id;
					return fullfill();
				})
	...
	game: function(req, res){
	    ...
	    setUpSocket().then(getGame).then(function(game){
	    	...
	    	sails.config.globals._2vs2['room' + gameId] = {
	    		times:0,
	    		users:[],
	    		game:[]
	    	};
		    		...
		    		Game.findOne({select: ['id','user_one','user_two','user_one_score', 'user_two_score'], where: {id:game.id} }).populate('questions',{limit:1,skip:skip}).exec(function(err,game){
						...
						Answer.find({select:['id','content'],where:{questionId:game.questions[0].id}}).exec(function(err, answers){
							...
							Game.update({id:game.id},{currentQuestion:game.questions[0].id}).exec(function(err,update){
						    	sails.config.globals._2vs2['room' + gameId].times = 0;
						    	sails.config.globals._2vs2['room' + gameId].users = [];
						    	sails.config.globals._2vs2['room' + gameId].game.push({
				    		...
				    		Answer.findOne({questionId:preQuesId, isTrue: true}).exec(function(err, correct){
					    		...
					    		GetAllQuestionAndAnswer().then(function(reviewQuestion){
					    			Game.findOne({select: ['id','user_one','user_two','user_one_score', 'user_two_score'], where: {id:game.id} }).exec(function(err, g){
					    				sails.sockets.broadcast('play-' + id1, 'receiveQuestion', {me_score:g.user_one_score,compatitor_score:g.user_two_score,correct:correct, gameReview:sails.config.globals._2vs2['room' + gameId].game,me:'u1',reviewQuestion: reviewQuestion});
										sails.sockets.broadcast('play-' + id2, 'receiveQuestion', {me_score:g.user_two_score,compatitor_score:g.user_one_score,correct:correct, gameReview:sails.config.globals._2vs2['room' + gameId].game,me:'u2',reviewQuestion: reviewQuestion});
										delete sails.config.globals._2vs2['room' + gameId];
	...
	gameAnswer: function(req, res){
		...
		Game.findOne({
			...
			if(err) return;
			if(!game) return;
			if(sails.config.globals._2vs2['room' + game.id].users.indexOf(req.session.passport.user) >= 0){
				return;
			}
			sails.config.globals._2vs2['room' + game.id].users.push(req.session.passport.user);
			Answer.findOne({questionId: game.currentQuestion,isTrue:true,id:req.body.answerId}).exec(function(err, ans){
				...
				var curQuesRoom = sails.config.globals._2vs2['room' + game.id].game.length - 1;
				if(game.user_one == req.session.passport.user){
					sails.config.globals._2vs2['room' + game.id].game[curQuesRoom].u1 = req.body.answerId;
				}else{
					sails.config.globals._2vs2['room' + game.id].game[curQuesRoom].u2 = req.body.answerId;
				}

				if(!ans){
					sails.config.globals._2vs2['room' + game.id].times = 0;
					return;
				}
				
				if(game.user_one == req.session.passport.user){
					if(sails.config.globals._2vs2['room' + game.id].times == 0 ){
						Game.update({id:game.id},{user_one_score:game.user_one_score + 150}).exec(function(err, update){});;
						sails.config.globals._2vs2['room' + game.id].times = new Date().getTime();
					}else{
						var times = new Date().getTime() - sails.config.globals._2vs2['room' + game.id].times;
	...
	playWithFriendJoinGame: function(req, res){
		...
		let SetUpPlayer = function(){
	    	return new Promise(function(fullfill, reject){
	    		Game.create({status:false, user_one:friendId, user_two:userId, type:2}).exec(function(err,game){
					if(err) return reject(err);
					gameId= game.id;
					return fullfill();
				})
	...
	history: function(req, res){
		...
		let countGame = function(){
	    	return new Promise(function(fullfill, reject){
	    		Game.count({
					or : [
						{ user_one:userId , currentQuestion:  null,type:type },
						{ user_two:userId , currentQuestion:  null,type:type }
					]
